...
// Simple health endpoint for Cloud Run / uptime checks
...
const verifyMiddleware = (req, res, next) => {
  ...
  // Allow Google / Cloud Run probes to bypass browser check
  if (ua.includes("GoogleHC") || ua.includes("Google-Cloud-Run")) {
    return next();
  }
...
app.get("/results/:query", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `http://api.duckduckgo.com/ac?q=${encodeURIComponent(
        query
      )}&format=json`
    );
...
app.get("/api/verify-email", (req, res) => {
  ...
  if (!token) {
    return res
      .status(400)
      .send("<html><body><h1>Invalid verification link</h1></body></html>");
  }
  try {
    const user = db
    ...
    return res
      .status(500)
      .send("<html><body><h1>Verification failed</h1></body></html>");
  }
});
...
app.get("/api/profile", (req, res) => {
  ...
  try {
    const user = db
      .prepare(
        "SELECT id, email, username, bio, avatar_url, is_admin, created_at FROM users WHERE id = ?"
      )
...
app.post("/api/update-profile", (req, res) => {
  ...
  try {
    ...
    const now = Date.now();
    db.prepare(
      "UPDATE users SET username = ?, bio = ?, age = ?, school = ? WHERE id = ?"
    ).run(
      username || null,
      bio || null,
      age || null,
...
app.post("/api/feedback", (req, res) => {
  ...
  try {
    ...
    if (!content || content.trim().length === 0) {
      return res
        .status(400)
        .json({ error: "Feedback content is required" });
    }
    const id = randomUUID();
    const now = Date.now();
    db.prepare(
...
app.get("/api/admin/stats", (req, res) => {
  ...
  try {
    ...
    const changelogCount = db
      .prepare("SELECT COUNT(*) as count FROM changelog")
      .get().count;
...
app.get("/api/admin/users", (req, res) => {
  ...
  try {
    const user = db
      .prepare("SELECT is_admin, email FROM users WHERE id = ?")
      .get(req.session.user.id);
    if (
      !user ||
      !(
...
app.post("/api/change-password", async (req, res) => {
  ...
  try {
    ...
    ).run(newPasswordHash, now, req.session.user.id);
    return res
      .status(200)
...
const handleHttpVerification = (req, res, next) => {
  // Allow health requests to go straight through
  ...
  res.writeHead(200, {
    ...
    "Set-Cookie":
      "verified=ok; Max-Age=86400; Path=/; HttpOnly; SameSite=Lax",
  });
  res.end(`
    <!DOCTYPE html>
    <html>
...
const handleUpgradeVerification = (req, socket, next) => {
  ...
  console.log(
    `WebSocket Upgrade Attempt: URL=${req.url}, Verified=${verified}, IsBrowser=${isWsBrowser}, Cookies=${
  ...
  console.log(
    `WebSocket Rejected: URL=${req.url}, Reason=${
      verified ? "Not a browser" : "Not verified"
    }`
  );
  socket.destroy();
};
